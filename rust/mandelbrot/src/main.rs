...
fn main() {
    ...
    let bounds = parse_pair(&args[2], 'x').expect("error parsing images dimensions");
...
/// Try to determine if `c` is in the Mandelbrot set, using the most `limit` iterations to decide.
///
...
fn espace_time(c: Complex<f64>, limit: u32) -> Option<u32> {
...
///Pasrse the string `s` as coordinate pair, like `"400x600"` or `"1.0,0.5"`.
///
/// Specifically, `s` should have the form <lefdt><sep><right>, where <sep> is
...
fn parse_pair<T: FromStr>(s: &str, separator: char) -> Option<(T, T)> {
    match s.find(separator) {
        None => None,
        Some(index) => {
            // Found separator, now split the string in tuple
...
/// `pixel` is a (column, row) pair indicationg a particular pixel in that image.
...
fn pixel_to_point(
    ...
    -> Complex<f64>
{
    let (wight, heigt) = (lower_right.re - upper_left.re, upper_left.im - lower_right.im);
    Complex {
        re: upper_left.re + pixel.0 as f64 * wight / bounds.0 as f64,
        im: upper_left.im - pixel.1 as f64 * heigt / bounds.1 as f64,
    ...
    lower_right: Complex<f64>)
{
    ...
    for row in 0..bounds.1 {
        for column in 0 ..bounds.0 {
            ...
            pixels[row * bounds.0 + column] = match espace_time(point, 255) {
                None => 0,
